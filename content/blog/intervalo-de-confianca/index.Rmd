...
```{r, echo=FALSE, fig.align='center', out.width = '450px', warning=FALSE, message=FALSE, fig.width=4.3, fig.height=3.6}
library(tidyverse)

ci_low <- function(media){
  media - 1.96*(15/sqrt(200))
}

ci_up <- function(media){
  media + 1.96*(15/sqrt(200))
}

set.seed(4321)
alturas <- rnorm(n = 500000, mean = 35.5, sd = 15)
pop <- as.data.frame(alturas)



# Sorteio e armazenamento dos resultados

res <- pop |> slice_sample(n = 200) |> 
  summarise(media = mean(alturas),
            cilow = ci_low(media),
            ciup = ci_up(media),
            x = 1) |> as.data.frame()
...
df_res <- df_res %>% mutate(cor = case_when(ciup > 35.5 & cilow > 35.5 ~ "#B5284B",
                                            ciup < 35.5 & cilow < 35.5 ~ "#B5284B",
                                            TRUE ~ "#2794A0"))


ggplot(df_res, aes(y = media, x = x, color = cor)) +
  geom_point(size = 1) +
  geom_errorbar(aes(ymin = cilow, ymax = ciup), width = 0) +
  geom_hline(aes(yintercept = 35.5), linetype = "dashed") +
  coord_flip() +
  labs(y = "Intenção de votos (%)", x = "Amostra") +
  scale_color_identity() +
  scale_y_continuous(labels = scales::number_format(decimal.mark = ",")) +
  theme_bw() +
  theme(text = element_text(size = 10, family = "Nunito"))
```
