...
<div class = "faq">
<center>
*Michelle Yeoh, vencedora do Oscar de Melhor atriz em 2023, aos 60 anos.*  
<font size='1' style="line-height:0.8;">Crédito de cortesia: Imagem do Executive Office of the President of USA (domínio público), via [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Michelle_Yeoh.jpg).</font>
</center>
</div></div>
  
<br><br>
  
>Esse post foi escrito em 2025 e atualizado em 16 de março de 2026, para incluir as idades dos vencedores da edição do Oscar de 2026.  
...
3. Preenchimento manual com base em pesquisa na Wikipedia das seguintes informações:
  - Os vencedores do Oscar de 2025 e 2026, que não constavam na base -- inclui seus nomes, nomes dos filmes e suas datas de nascimento.
...
A média de idade ao receber o prêmio de melhor atriz, durante todo o período (1927 a 2026) foi de **36,9 anos** (*versus* **44,6 anos** para melhor ator). Esse padrão se mantém quando olhamos para a mediana: 34 anos para as mulheres e 43 anos para os homens. A interpretação desse dado é: metade das mulheres que ganharam o Oscar de melhor atriz tinham até 34 anos no momento da premiação. Para homens, esse valor aumenta em quase 10 anos: metade dos homens premiados nessa categoria tinham 43 anos ou menos. Isso mostra que os **homens** que ganham o prêmio máximo de atuação são, em geral, **mais velhos** que as mulheres.  
  
Quando avaliamos os prêmios de atrizes e atores coadjuvantes, novamente a média de idade das mulheres é menor: **41,2 anos** *versus* **50,0 anos**. A mediana, mais uma vez, segue a tendência da média. Metade das mulheres premiadas na categoria de atuação coadjuvante tinham até 39 anos no momento da premiação. Entre os homens, metade dos premiados tinham até 46 anos.  
  
Também adicionei à tabela os percentis 25 e 75 (P25 e P75). Para a categoria de melhor atriz, o P25 é 28,5 anos e o P75 é de 41 anos. Isso significa que **25% das mulheres que venceram nessa categoria têm até 28 anos**; além disso, 2/3 dessas mulheres (75%) têm até 41 anos. Isso também significa que **apenas 25% das premiadas como "melhor atriz" tinham mais de 40 anos**. Para os vencedores do prêmio de "melhor ator", P25 e P75 são de 38 e 50,5 anos, respectivamente.  
...
dados |> 
  ...
  summarise(`Média` = mean(`Idade (anos) na cerimônia`),
            ...
            P25 = quantile(`Idade (anos) na cerimônia`, probs = 0.25),
            Mediana = median(`Idade (anos) na cerimônia`),
            P75 = quantile(`Idade (anos) na cerimônia`, probs = 0.75)) |> 
  mutate(across(c(2:3,6:8), \(x) scales::number(x, accuracy = 0.1,
                                            decimal.mark = ","))) |> 
  flextable::flextable() |> 
  ...
  flextable::autofit()
```
<font size='2'>P25 = Percentil 25; P75 = Percentil 75.</font>
...
# dados |> 
#   group_by(Categoria) |> 
#   summarise(Moda = DescTools::Mode(`Idade (anos) na cerimônia`)[1],
#             Frequência = attr(DescTools::Mode(`Idade (anos) na cerimônia`),
#                               which = "freq")) |> 
#   flextable::flextable() |> 
#   flextable::bold(part = "header") |> 
#   flextable::align(j = -c(1), align = "center", part = "all") |> 
...
No nosso caso, a **população** é composta por **todos os ganhadores do Oscar** de atuação, no período de 1927 a 2026. E, sim, nós temos acesso a toda a população. Portanto, não precisamos utilizar recursos para fazer inferências. Nossa pergunta é respondida com uma simples análise descritiva.  
...
ggplot(dados, aes(y = `Idade (anos) na cerimônia`, x = Categoria)) +
...
dados |> 
  group_by(Categoria) |> 
  arrange(`Ano da cerimônia`) |> 
  mutate(`Idade média` = zoo::rollmean(`Idade (anos) na cerimônia`,
                                       k = 15, fill = NA, align = "center")) |> 
  ggplot(aes(x = `Ano da cerimônia`,
             y = `Idade média`,
             color = Categoria)) +
...
> PERES, Fernanda F. **As idades de atores e atrizes vencedores do Oscar diferem?**. Blog Fernanda Peres, São Paulo, 05 mar. 2025. Atualizado em 16 mar. 2026. Disponível em: https://fernandafperes.com.br/blog/oscar-idades/.
