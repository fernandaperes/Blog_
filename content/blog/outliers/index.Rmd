...
ggplot(dados, aes(y = Idade, x = "")) +
  geom_dotplot(binaxis = "y", stackdir = "center", fill = azul,
               binwidth = 1, color = azul, alpha = 0.5, dotsize = 1.3) +
...
Portanto, a AIQ nessa amostra é 3:  
...
<center>
Limite inferior teórico = Q1 - 1,5 x AIQ  
Limite inferior teórico = 18 - 1,5 x 3  
Limite inferior teórico = 18 - 4,5  
**Limite inferior teórico = 13,5**  
  
  
Limite superior teórico = Q3 + 1,5 x AIQ  
Limite superior teórico = 21 + 1,5 x 3  
Limite superior teórico = 21 + 4,5  
**Limite superior teórico = 25,5**  
  
</center>
  
  
Ou seja, de acordo com a definição de *outlier* utilizada pelo boxplot de Tukey, qualquer valor **abaixo de 13,5 anos** ou **acima de 25,5 anos** será considerado um *outlier* nessa amostra de idades. A idade de 75 anos, portanto, será considerada um *outlier* de acordo com esse critério.  
...
Até aqui você pode estar pensando: "ok, tem um valor discrepante na amostra, e daí?". A nossa maior **preocupação** quando temos um *outlier* **univariado** na amostra é com relação à **média** e às medidas derivadas dela, como o **desvio-padrão**. A média é uma medida muito **influenciada por valores extremos** -- o que significa que na presença desses valores ela pode **não ser** uma boa representação do conjunto de dados.  
  
Para isso ficar menos abstrato, vou retomar o exemplo das idades. Nós vimos que a média da amostra é de **23,7 anos**. Se eu te digo: "olha, tenho uma turma de pré-vestibular com idade média de 23,7 anos", **o que viria à sua cabeça?** Eu mesma imaginaria que os alunos tem em torno de 24 anos, com idades variando entre 20 e 28. Mas veja: isso está bem longe da realidade da nossa amostra.  
...
ggplot(dados |> filter(Idade < 30), aes(y = Idade, x = "")) +
  geom_dotplot(binaxis = "y", stackdir = "center", fill = azul,
               binwidth = 1, color = azul, alpha = 0.5, dotsize = 0.22) +
  geom_crossbar(stat = "summary", fun = "mean", middle.linewidth = 0.5,
                width = 0.3) +
...
>  
>Abaixo eu vou explicar os conceitos de *outliers* **bivariados** e **multivariados**. Se você não tem interesse nesse tema, mas quer saber como lidar com os *outliers* das suas análises, recomendo pular para <a href="#como-lidar-com-outliers">essa seção</a>.  
>  
...
Até agora te expliquei como identificar um *outlier* univariado. Mas muitas vezes -- principalmente ao rodarmos testes de hipóteses ou modelos de regressão -- a nossa preocupação é com os *outliers* **bivariados**. Então, vamos entender isso melhor.  
  
  
#### Variável numérica x categórica
  
O exemplo acima envolve duas variáveis numéricas. Mas podemos também detectar um *outlier* bivariado ao considerarmos simultaneamente uma variável numérica e uma categórica.  
  
Vamos de novo partir para um exemplo. Veja as pressões arteriais abaixo. Perceba que as pressões variam bastante -- de 94 a 154 mmHg -- mas que nenhum ponto se destaca. Ou seja: **não** temos *outliers* **univariados**.  
  
```{r, fig.width=2.5, echo=FALSE}
library(tibble)

set.seed(4321)
dados <- tibble(
  faixa_etaria = c(rep("Jovem", 20),
                   rep("Idoso", 20)),
  pressao = c(
    round(rnorm(n = 20, mean = 110, sd = 10), 0),
    round(rnorm(n = 20, mean = 135, sd = 15), 0)
  )
)

dados$pressao[20] <- 140


ggplot(dados, aes(y = pressao, x = "")) +
  ggbeeswarm::geom_beeswarm(color = azul, alpha = 0.5,
                            method = "center", cex = 2.5) +
  labs(x = NULL, y = "Pressão sistólica (mmHg)") +
  theme_classic() +
  theme(text = element_text(family = "Nunito"),
        axis.ticks.x = element_blank())
```
  
De fato, ao construirmos um boxplot com esses dados, nenhum ponto é considerado um *outlier*:  
  
```{r, fig.width=2.5, echo=FALSE}
ggplot(dados, aes(y = pressao, x = "")) +
  geom_errorbar(stat = "boxplot", width = 0.3) +
  geom_boxplot(fill = azul) +
  labs(x = NULL, y = "Pressão sistólica (mmHg)") +
  theme_classic() +
  theme(text = element_text(family = "Nunito"),
        axis.ticks.x = element_blank())
...
Ok, mas agora vou te dar mais uma informação: essa base de dados é formada por **dois grupos**, jovens e idosos. E eu quero avaliar a pressão arterial de acordo com o grupo. O que muda?  
  
```{r, fig.width=4.5, echo=FALSE}
ggplot(dados, aes(y = pressao, x = faixa_etaria)) +
  ggbeeswarm::geom_beeswarm(color = azul, alpha = 0.5,
                            method = "center", cex = 2.5) +
  labs(x = "Grupo", y = "Pressão sistólica (mmHg)") +
  theme_classic() +
  theme(text = element_text(family = "Nunito"),
        axis.ticks.x = element_blank())
```
  
Observe que agora um ponto **passa a se destacar no grupo dos jovens**: uma pessoa com a pressão arterial de 140 mmHg, bem superior às outras pressões desse grupo. De fato, ao construirmos um boxplot esse ponto é identificado como um *outlier*:  
  
```{r, fig.width=4.5, echo=FALSE}
ggplot(dados, aes(y = pressao, x = faixa_etaria)) +
  geom_errorbar(stat = "boxplot", width = 0.3) +
  geom_boxplot(fill = azul, outlier.shape = 1) +
  labs(x = "Grupo", y = "Pressão sistólica (mmHg)") +
  theme_classic() +
  theme(text = element_text(family = "Nunito"),
        axis.ticks.x = element_blank())
...
Portanto, esse ponto é um *outlier* **bivariado** -- que só passa a ser considerado *outlier* quando consideramos **simultaneamente duas variáveis**: a pressão e a faixa etária.  
  
  
#### Variável numérica x numérica
...
Não parece um *outlier*, certo? Tanto a idade quanto a pressão estão dentro do intervalo observado nos demais dados.  
  
De fato, quando avaliamos esse ponto (destacado em rosa) considerando apenas a pressão (gráfico à esquerda) ou apenas a idade (gráfico à direita), esse ponto não destoa dos demais. Portanto, esse ponto não pode ser considerado um *outlier* **univariado**:  
...
a <- ggplot(df2, aes(y = pressao, x = "", color = cor)) +
  geom_point(show.legend = F, alpha = 0.5) +
  scale_y_continuous(labels = scales::number_format(decimal.mark = ",")) +
  scale_color_manual(values = c(azul, rosa)) +
  labs(y = "Pressão arterial sistólica (mmHg)", x = NULL) +
  theme_classic() +
  theme(text = element_text(family = "Nunito"),
        axis.ticks.x = element_blank())

b <- ggplot(df2, aes(y = idade, x = "", color = cor)) +
  geom_point(show.legend = F, alpha = 0.5) +
  scale_y_continuous(labels = scales::number_format(decimal.mark = ",")) +
  scale_color_manual(values = c(azul, rosa)) +
...
No entanto, veja o que acontece quando consideramos **simultaneamente** a idade e a pressão:  
...
No gráfico de dispersão conseguimos perceber que essa nova pessoa (ponto em rosa) **destoa do padrão** das demais: trata-se de alguém com pressão arterial mais alta do que o padrão observado para a sua idade. Portanto, a pessoa em rosa é um *outlier* -- mas um ***outlier* bivariado**, que só passa a ser considerado *outlier* quando analisamos simultaneamente a idade e a pressão.  
  
Nem todo *outlier* representa um **problema** no contexto da **regressão**. O problema aparece quando esse ponto é **influente** -- ou seja, quando influencia, modifica, as estimativas da reta de regressão.  
  
Note que o *outlier* em rosa não modifica de forma significativa as estimativas da reta: a reta estimada com ele (em preto, linha sólida) se assemelha muito à reta estimada sem ele (pontilhada).  
...
Ok, mas o ponto de alavancagem é um **problema** para a **regressão**? Não necessariamente. Isso porque ele pode ser um ponto de alavancagem, mas não influenciar as estimativas da reta -- ou seja, não ser um **ponto influente**. Note que no exemplo o ponto em rosa não modifica de forma significativa a reta de regressão: a reta estimada com ele (linha preta sólida) é muito semelhante à estimada sem ele (linha pontilhada).  
...
Como já discutimos, um **ponto influente** é aquele que **modifica as estimativas da reta** de regressão. Vamos voltar ao nosso exemplo e adicionar à amostra (em rosa) uma pessoa com idade de **96 anos** e pressão arterial de **127 mmHg**:  
...
Uma vez que esse ponto é tanto um ponto de alavancagem quanto um *outlier* bivariado, ele influencia as estimativas da reta, é um **ponto influente**. Perceba como a reta estimada com esse ponto (linha sólida em preto) **difere muito** da reta estimada quando ele é excluído (linha pontilhada):  
...
Para isso fazer mais sentido, vamos voltar à amostra representada no gráfico abaixo. Vimos que o ponto em rosa é um ponto influente, uma vez que a sua inclusão na amostra modifica as estimativas da reta:  
...
Se quisermos comparar estatisticamente esses grupos, a análise clássica a ser utilizada é o teste-t independente. No entanto, **o teste-t nada mais é do que um modelo linear**, como a regressão. A ANOVA, utilizada para comparar as médias de mais de dois grupos, é também um tipo de modelo linear. Isso significa que tudo o que discutimos para regressão se aplica aqui: para esse cenário também podemos calcular a **distância de Cook** para avaliar a **influência** desse *outlier* nas estimativas do modelo.  
  
Note que o **jovem** com **140 mmHg** de pressão arterial apresenta a maior distância de Cook -- mas ela está abaixo do ponto de corte sugerido de 0,5.  
  
```{r}
mod <- lm(pressao ~ faixa_etaria, data = dados)

tab_cook <- dados |> 
  select(c(faixa_etaria, pressao)) |> 
...
linha_vazia <- tab_cook1 |>
  filter(`Distância de Cook` == "0,054") |> 
  mutate(across(everything(), \(x) x = "..."))
...
Em **análises descritivas**, faz sentido nos preocuparmos com ***outliers* univariados**, já que estamos olhando para o comportamento de cada variável isoladamente. Ah, nos casos em que a análise descritiva envolve grupos, a avaliação deve ser feita **dentro de cada grupo**, ok?  
  
Por outro lado, em análises inferenciais, como **regressão**, **teste-t**, **ANOVA**, remover *outliers* univariados **não faz sentido**. Um ponto pode ter um valor discrepante em uma variável e, ainda assim, não destoar do padrão dos dados. Vimos isso no exemplo de <a href="#caso-2-ponto-de-alavancagem">ponto de alavancagem</a>.  
Para esse tipo de análise, o foco não deve estar em *outliers* univariados, mas sim em <a href="#caso-3-ponto-influente">**pontos influentes**</a> -- isto é, observações que têm impacto relevante sobre as estimativas do modelo. Ou seja, nesses cenários vamos avaliar a distância de Cook ou alguma outra medida de influência.  
  
  
#### Passo 2: entender por que aquela observação é um *outlier*
  
Identificado um possível ponto discrepante (seja um *outlier* ou um ponto influente, a depender do seu objetivo), o passo seguinte é avaliar por que aquela observação difere das demais:  
...
>Caso você opte por remover *outliers* da sua amostra, essa decisão deve estar descrita no seu texto -- seja em um artigo, dissertação, TCC ou tese. É fundamental **justificar** essa escolha e informar **quantas observações** foram removidas. Esse nível de transparência é fundamental para a **avaliação crítica** e a **reprodutibilidade** dos resultados.  
  
  
Agora, pense em um cenário diferente: suponha que o *outlier* identificado corresponda a um valor **real** e **plausível**, ainda que raro. É o caso do aluno de 75 anos em uma turma de pré-vestibular, exemplo que abriu este post. Nesse caso, a lógica muda completamente. Quando o valor é **real**, **não devemos excluí-lo**.  
  
No caso de **análises descritivas**, uma estratégia mais adequada é complementar a análise com **medidas menos sensíveis** a esse valor extremo. Além da média -- que, como discutimos, é fortemente influenciada por *outliers* -- podemos reportar a mediana, uma medida robusta a eles. Da mesma forma, além do desvio-padrão -- uma medida derivada da média e, portanto, também sensível a valores extremos -- podemos calcular medidas de dispersão mais robustas, como a amplitude interquartil (AIQ).  
  
Quando pensamos em **testes estatísticos**, nossas opções são:  
  
* Utilizar testes mais **robustos** a valores extremos, como é o caso dos testes não-paramétricos
* Utilizar testes mais adequados à **distribuição dos dados**, como os GLMs (Modelos Lineares Generalizados)
* Fazer uma análise de sensibilidade: estimar o modelo **com e sem** a observação influente e, caso as conclusões sejam diferentes, discutir esse impacto no trabalho
